...
9. **Homepage/TER-level coverage for most AMCs:** `urls.csv` carries verified homepage and Total Expense Ratio sources for 25 major fund houses, and scheme-level depth (individual scheme pages, factsheets, SID/KIM PDFs) for 10 of them — SBI, HDFC, ICICI Prudential, Nippon India, Aditya Birla Sun Life, Kotak Mahindra, Axis, UTI, DSP, and Tata's most popular funds. The remaining 15 AMCs will answer TER/general questions but likely hit "not found" for scheme-specific facts until their own scheme-level sources are added (same pattern as the existing rows in `urls.csv`).
